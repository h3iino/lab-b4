...
class CNN_AutoEncoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.Encoder = nn.Sequential(  # in(3*256*256)
            ...
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.Conv2d(64, 64, kernel_size=5, stride=2, padding=2),  # out(16*32*32)
            # nn.Conv2d(16, 64, kernel_size=5, stride=4, padding=2),  # out(16*16*16)
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.Conv2d(64, 32, kernel_size=5, stride=2, padding=2),  # out(24*16*16)
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),
        ...
        self.Decoder = nn.Sequential(
            ...
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(32, 64, kernel_size=2, stride=2),  # out(16*32*32)
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(64, 64, kernel_size=4, stride=4),  # out(16*64*64)
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
